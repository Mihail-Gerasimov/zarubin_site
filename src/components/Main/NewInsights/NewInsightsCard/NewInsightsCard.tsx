...
export const NewInsightsCard = ({
  ...
  return (
    <Link
      ...
      className='flex h-full flex-col pr-[10px]'
    >
      <div className='relative flex aspect-[16/9] w-full overflow-hidden'>
        {image ? (
          <Image
            src={image}
            alt={title}
      ...
      <div className='flex h-full w-full flex-1 flex-col items-stretch rounded-b-[5px] bg-card-bg px-[20px] py-[40px] tablet:px-[40px]'>
        ...
        <NextLinePreposition
          tag='h3'
          text={title}
          className='mt-[25px] line-clamp-3 overflow-hidden font-unbound text-[18px] font-bold uppercase leading-[1] text-text-dark tablet:text-[24px] tablet:leading-[1.16]'
        />
        <NextLinePreposition
          tag='p'
          text={description}
